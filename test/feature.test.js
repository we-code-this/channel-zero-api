...
describe('features', function () {
  ...
  describe('GET /feature', function () {
    ...
    it("should return feature with an article property that's an object", async function () {
      ...
      expect(
        JSON.parse(response.payload).article,
    ...
    it("should return feature with a video property that's an object", async function () {
      ...
      expect(JSON.parse(response.payload).video).to.be.an.instanceof(
        Object,
      );
    });
  });
  ...
  describe('GET /feature/:id', function () {
    it('should return the feature that has the :id supplied', async function () {
      ...
      expect(JSON.parse(response.payload).article.title).to.equal(
        'Article 1',
      );
  ...
  describe('POST /feature', function () {
    it('should add feature record to database', async function () {
      ...
      expect(JSON.parse(feature.payload).article.title).to.equal(
        'Article 1',
      );
  ...
  describe('PATCH /feature/publish', function () {
    it('should publish an unpublished feature', async function () {
      const token = await login(app);
      const articleId = 2;
      ...
      const feature = JSON.parse(featureRes.payload);

      expect(feature.published).not.to.be.true;
      ...
      expect(JSON.parse(updatedFeature.payload).published).to.be.true;
    });
  });
  ...
  describe('PATCH /feature/unpublish', function () {
    it('should unpublish a published feature', async function () {
      ...
      expect(feature.published).to.be.true;

      const updatedFeature = await app.inject({
        method: 'PATCH',
        url: '/feature/unpublish',
        payload: {
          id: feature.id,
        },
        headers: {
      ...
      expect(JSON.parse(updatedFeature.payload).published).not.to.be
        .true;
    });
  });
